...
class SmartInstantVoiceNotepad:
    def __init__(self, root):
        self.root = root
        self.root.title("🎤 УМНЫЙ МГНОВЕННЫЙ ГОЛОСОВОЙ БЛОКНОТ")
        ...
        # Заголовок
        title_label = ttk.Label(root, text="🎤 АРДУМГНОВЕННЫЙ ГОЛОСОВОЙ БЛОКНОТ", style='Title.TLabel')
...
4. Если вставка не сработала — нажмите F12 (работает даже когда окно свёрнуто!).
5. Используйте 'Исправить' для авто-пунктуации и 'Очистить' для сброса."""
    ...
    def paste_text_global(self):
        ...
        try:
            ...
            if self.root.winfo_exists():
                self.status_var.set(f"❌ Ошибка вставки F12: {str(e)}")

    def paste_text(self):
        """Вставить через кнопку (аналог F12)"""
            ...
            while self.listening:
                try:
                    ...
                    if self.instant_paste(processed_text.strip()):
                        ...
                        self.root.after(0, lambda: self.status_var.set("⚠️ Не удалось вставить — попробуйте F12"))
